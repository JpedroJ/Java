...
public class DespesasView extends JFrame {
	...
	private int codigoDespesaEmEdicao;
	...
	public DespesasView() {
		...
		tblDados.getColumnModel().getColumn(4).setPreferredWidth(100);  // Data Pagamento
		tblDados.getColumnModel().getColumn(4).setResizable(false);
		tblDados.removeColumn(tblDados.getColumnModel().getColumn(0));  // Oculta a coluna Código
	...
	private void incluirDespesa() {
		...
		if (categoriaStr.equals("--selecione--") ||
			descricaoStr.isEmpty() ||
			valorStr.isEmpty() ||
		...
		JOptionPane.showMessageDialog(this,
                "Os dados da despesa foram incluídos com sucesso!",
	...
	private void salvarEdicaoDespesa() {
		...
		JOptionPane.showMessageDialog(this,
				                      "Os dados da despesa foram salvos com sucesso!",
				                      "Operação realizada com sucesso",
				                      JOptionPane.INFORMATION_MESSAGE);
	}
	...
	private void editarDespesa() {
		if (tblDados.getSelectedRow() > -1) {
	    	...
	    	String dataPagamento = (String)dtm.getValueAt(tblDados.getSelectedRow(), 4);
    	
	    	fldCategoria.setSelectedItem(categoria);
	    	fldDescricao.setText(descricao);
	    	fldValor.setText(valor);
	    	fldDataPagamento.setText(dataPagamento);
	...
	private void excluirDespesa() {
	    if (tblDados.getSelectedRow() > -1) {
	    	int resposta = JOptionPane.showConfirmDialog(null,
	    			                                     "Deseja mesmo excluir a despesa selecionada?",
	    			                                     "Excluir despesa",
	        ...
	        if (resposta == JOptionPane.YES_OPTION) {
	          ...
	          JOptionPane.showMessageDialog(null, 
	            		                      "Os dados da despesa foram excluídos com sucesso!",
	            		                      "Operação realizada com sucesso!",
	            		                      JOptionPane.INFORMATION_MESSAGE);
	        }
	      }
	...
	private void preencherTabelaDespesas() {
		List<DespesaModel> despesas = controller.listar();
		...
		for (int i=0; i < despesas.size(); i++) {
			...
			dtm.addRow(new Object[] {despesa.getCodigo(),
					                 despesa.getCategoria(),
			                         despesa.getDescricao(),
			                         String.valueOf(despesa.getValor()),
			                         sdf.format(despesa.getDataPagamento())} );
			}
